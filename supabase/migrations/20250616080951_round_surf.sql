/*
  # Fix user roles and role-based access control

  1. Changes
    - Ensure role column exists in user_profiles table
    - Add check constraint for valid roles if needed
    - Create policy for role updates with proper syntax
...
-- Fixed syntax to avoid using OLD reference directly
CREATE POLICY "Only superadmins can update roles" 
ON public.user_profiles
FOR UPDATE
USING (
  ((uid() = id) OR 
   EXISTS (SELECT 1 FROM public.user_profiles WHERE id = uid() AND role = 'superadmin'))
)
WITH CHECK (
  ((uid() = id) AND (
    ...
    EXISTS (
      ...
      WHERE id = uid() 
      AND role = (SELECT role FROM public.user_profiles WHERE id = uid())
    )
  )) OR 
